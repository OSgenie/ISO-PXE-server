...
MENU LABEL $revision
    ...
    append initrd=$kernelpath/$distro_ram_disk noprompt boot=$boot_folder url=$seed_path/$seed_file netboot=nfs root=/dev/nfs nfsroot=$nfs_root_path/$distro/$revision ro toram -
